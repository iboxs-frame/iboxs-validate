...
/**
 * @see \iboxs\Validate
 * @mixin \iboxs\Validate
 * @method static \iboxs\Validate rule(mixed $name, mixed $rule = '') 添加字段验证规则
 ...
 * @method static \iboxs\Validate message(mixed $name, string $message = '') 设置提示信息
 * @method static \iboxs\Validate scene(string $name) 设置验证场景
 * @method static bool hasScene(string $name) 判断是否存在某个验证场景
 * @method static \iboxs\Validate batch(bool $batch = true) 设置批量验证
 * @method static \iboxs\Validate only(array $fields) 指定需要验证的字段列表
 * @method static \iboxs\Validate remove(mixed $field, mixed $rule = true) 移除某个字段的验证规则
 * @method static \iboxs\Validate append(mixed $field, mixed $rule = null) 追加某个字段的验证规则
